...
const BlogsCategory_Schema = require("../modals/BlogsCategory");
...
const getAllBrands = async (req,res) => {
  try {
    const result = await BlogsCategory_Schema.find();
...
const createCategory = async (req, res) => {
  ...
  try {
    const findExistingCategory = await BlogsCategory_Schema.find({
    ...
    const findMainCategory = await BlogsCategory_Schema.find({
    ...
    if (findMainCategory[0]?.category_name) {
      ...
      const create = new BlogsCategory_Schema({
      ...
      const updateCategory = await BlogsCategory_Schema.findByIdAndUpdate(
...
const editCategory = async (req, res) => {
  ...
  try {
    ...
    const findCategory = await BlogsCategory_Schema.findByIdAndUpdate(categoryId, {
      $set: req.body,
    });
    if (!findCategory) {
...
const updateMainCategory = async (req, res) => {
  ...
  try {
    if (oldMainCategory) {
      const find = await BlogsCategory_Schema.find({
        main_category_name: oldMainCategory,
      });
      console.log("FIND___", find);
      if (find?.length > 0) {
        const updateAll = await BlogsCategory_Schema.updateMany(
          { main_category_name: oldMainCategory },
          {
            $set: {
...
const deleteImage = async (req, res) => {
  ...
  try {
    if (oldMainCategory) {
      ...
      if (find?.length > 0) {
        const removeImage = await BlogsCategory_Schema.updateMany(
          { main_category_name: oldMainCategory },
          {
            $set: {
              main_category_image: {},
    ...
    if (oldCategory) {
      const findCategory = await BlogsCategory_Schema.find({
    ...
    if (oldSubId && oldSubCategory) {
      ...
      const findSubCategroy = await BlogsCategory_Schema.find({
        _id: oldSubId,
      ...
      if (findSubCategroy?.length > 0) {
        ...
        const updateSubCategory = await BlogsCategory_Schema.findByIdAndUpdate(
          { _id: oldSubId },
...
const getAllCategory = async (req, res) => {
  try {
    const countCategory = await BlogsCategory_Schema.find({}).count();
    const categoryForFilter = await BlogsCategory_Schema.aggregate([
      { $group: { _id: "$main_category_name" } },
    ]).sort({ _id: 1 });
    //  console.log(categoryForFilter)
    const allCategory = await BlogsCategory_Schema.find({});
    res
      .status(200)
      .send({
...
const getCategoryById = async (req, res) => {
  ...
  try {
    ...
    const getCategoires = await BlogsCategory_Schema.findById(cat_id);
    if (!getCategoires) {
...
const filterForCategory = async (req, res) => {
  ...
  try {
    if (main_category) {
      result = await BlogsCategory_Schema.find({ main_category_name: main_category });
      console.log(result);
...
const searchInCategory = async (req, res) => {
  ...
  try {
    const countCategory = await BlogsCategory_Schema.find({}).count();
    result = await BlogsCategory_Schema.find({
...
const getCategoryByMainCategory = async (req, res) => {
  ...
  try {
    if (main_category) {
      const findCategory = await BlogsCategory_Schema.aggregate([
    ...
    // if(categoryID){
    //     const findSubCategroy= await BlogsCategory_Schema.findById(categoryID)
...
const deleteCategory = async (req, res) => {
  ...
  try {
    if (req.body?.length) {
      const deleteSelected = await BlogsCategory_Schema.deleteMany({
        _id: {
          $in: req.body,
        },
      });
